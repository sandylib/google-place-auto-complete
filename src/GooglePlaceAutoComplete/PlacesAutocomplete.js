...
import { compose } from '../utils/helps';
...
class PlacesAutocomplete extends React.Component {
  ...
  init = () => {
    if (!window.google) {
      throw new Error(
        '[AutocompleteService]: google map required'
      );
    }

    if (!window.google.maps.places) {
      throw new Error(
        '[AutocompleteService]:google map places lib required '
      );
    }
      ...
      this.autocompleteService.getPlacePredictions(
        {
          ...
          input: value,
          componentRestrictions: {country: this.props.restrictionedCountry}
        },
  ...
  getInputProps = (options = {}) => {
    if (options.hasOwnProperty('value')) {
      throw new Error(
        '[google-place-auto-complete]: getInputProps does not accept `value`. Use `value` prop instead'
      );
    }

    if (options.hasOwnProperty('onChange')) {
      throw new Error(
        '[google-place-auto-complete]: getInputProps does not accept `onChange`. Use `onChange` prop instead'
      );
    }
...
PlacesAutocomplete.defaultProps = {
  ...
  onError: (status, _clearSuggestions) =>
    console.error(
      '[google-place-auto-complete]: error happened when fetching data from Google Maps API.\nPlease check the docs here (https://developers.google.com/maps/documentation/javascript/places#place_details_responses)\nStatus: ',
      status
    ),
